// **********************************************************
// Assignment2:
// Student 1:
// UTORID: REDACTED
// UT Student #: REDACTED
// Author: REDACTED
//
// Student 2:
/// UTORID: REDACTED
// UT Student #: REDACTED
// Author: REDACTED
//
// Student 3:
// Utorid :REDACTED
// UT Student #: REDACTED
// Author : REDACTED
//
// Student 4:
// Utorid :REDACTED
// UT Student #: REDACTED
// Author : Vihanga Ratnasinghe
//
// Honor Code: I pledge that this program represents my own
// program code and that I have coded on my own. I received
// help from no one in designing and debugging my program.
// I have also read the plagiarism section in the course info
// sheet of CSC B07 and understand the consequences. In this semester
// we will select any three of your assignments from total of 5 and run it
// for plagiarism check.
// *********************************************************

package a2;

import driver.JCmdManager;

public class Command implements ICommand{
  /**
   * Creates a new console to be returned in getShowUser
   */
  private Console showUser = new Console();

  private IFileManager FileSystem = FileManager.getInstance();

  /**
   * returns the console to display contents
   * 
   * @return showUser the console to show user
   */
  public Console getShowUser() {
    return showUser;
  }

  /**
   * Executes command
   * 
   * @param cmdArgs the array of the input command splitted by spaces
   * @return 0 symbolizing successfully executed
   */
  @Override
  public String exec(String[] cmdArgs) {
    // TODO Auto-generated method stub
    return "";
  }
  
  /**
   * redirects output to file
   * 
   * @param cmdArgs the array of the input command splitted by spaces
   */
  public void redirect(String output, String sign, String outfile) {
    String[] cmd = new String[] {"echo", output, sign, outfile};
    JCmdManager commandexec = new JCmdManager();
    commandexec.select(cmd);
  }
  
  /**
   * returns file system instance
   * 
   * @return instance of file system
   */
  public IFileManager getFileSystem() {
    return this.FileSystem;
  }

}
